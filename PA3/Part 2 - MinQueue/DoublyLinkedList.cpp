...
void DoublyLinkedList::insertAfter(DListNode &p, int newobj)
{
  ...
  if(p.next->next == nullptr){
    throw std::range_error("Cannot insert after trailer");
  } else {
    newNode->prev = &p;
    newNode->next = p.next;
    p.next->prev = newNode;
...
void DoublyLinkedList::insertBefore(DListNode &p, int newobj)
{
  DListNode* newNode = new DListNode(newobj);
  if(p.prev == nullptr){
    throw std::range_error("Cannot insert before header");
  } else {
    newNode->next = &p;
    newNode->prev = p.prev;
    p.prev->next = newNode;
...
int DoublyLinkedList::removeAfter(DListNode &p)
{
    if (p.next == nullptr || p.next->next == nullptr) {
      throw std::range_error("Cannot delete trailer or after trailer");
    } else {
...
int DoublyLinkedList::removeBefore(DListNode &p)
{
    if (p.prev == nullptr || p.prev->prev == nullptr) {
      throw std::range_error("Cannot delete header or before header");
    } else {
